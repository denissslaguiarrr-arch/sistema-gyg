...
(function (global) {
  ...
  async function fetchViaRaw(gistId, filename) {
    ...
    for (let i = 0; i < urls.length; i += 1) {
      try {
        return await fetchJsonUrl(urls[i]);
      } catch (err) {
        lastErr = err;
      }
    }
    throw lastErr || new Error("No se pudo descargar el catálogo");
  }
  ...
  async function loadStock() {
    ...
    for (let i = 0; i < ids.length; i += 1) {
      ...
      for (let intento = 0; intento < 3; intento += 1) {
        try {
          return normalizeData(await loadGistOnce(gistId, filename));
        } catch (err) {
          lastErr = err;
          await sleep(280 * (intento + 1));
        }
      }
      if (i === 0 && ids.length > 1) clearStoredGistId();
    }
...
(function () {
  ...
  async function init() {
    try {
      ...
      console.error(err);
      if (!err || err.code !== "MISSING_GIST_ID") {
        var auto = 0;
        try {
          auto = Number(sessionStorage.getItem("gyg_gist_auto_retry") || "0");
        } catch (_) {
          auto = 0;
        }
        if (auto < 2) {
          try {
            sessionStorage.setItem("gyg_gist_auto_retry", String(auto + 1));
          } catch (_) {}
          GyGStock.clearStoredGistId();
          app.innerHTML =
            '<section class="empty-state" style="margin:3rem 1.25rem;max-width:36rem">' +
            "<h1>Cargando el catálogo…</h1>" +
            "<p>Un momento, lo estamos bajando de nuevo.</p>" +
            "</section>";
          setTimeout(function () {
            location.reload();
          }, 2200);
      ...
      renderGistSetup(err);
      toast("No se pudo cargar el catálogo. Recargá en un momento.", "error");
    }
  }
