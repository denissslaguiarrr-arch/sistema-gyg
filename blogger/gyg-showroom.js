...
window.GYG_CONFIG = {
  // Obligatorio en el sitio público: el ID del Gist con stock.json.
  // Sin esto, los visitantes no ven el catálogo.
  GIST_ID: "",
  GIST_OWNER: "",
...
(function (global) {
  ...
  const CACHE_KEY = "gyg_stock_cache";
  ...
  function embeddedStock() {
    if (global.GYG_STOCK_FALLBACK && typeof global.GYG_STOCK_FALLBACK === "object") {
      return global.GYG_STOCK_FALLBACK;
    }
    return null;
  }

  function stockOnHand() {
    return readStockCache() || embeddedStock();
  }
  ...
  function fetchWithTimeout(url, ms) {
    ...
    return fetch(url, { cache: "no-store", credentials: "omit", signal: ctrl.signal }).finally(() => {
      clearTimeout(t);
    });
  }
  ...
  async function fetchJsonUrl(url) {
    ...
    try {
      ...
      throw new Error("El catálogo no es JSON válido");
    }
  }

  function firstOk(producers) {
    return new Promise((resolve, reject) => {
      const list = Array.isArray(producers) ? producers : [];
      let pending = list.length;
      let lastErr;
      if (!pending) {
        reject(new Error("No se pudo descargar el catálogo"));
        return;
      }
      let settled = false;
      list.forEach((make) => {
        Promise.resolve()
          ...
          .catch((err) => {
            lastErr = err;
            pending -= 1;
            if (!settled && pending < 1) reject(lastErr || new Error("No se pudo descargar el catálogo"));
          });
      });
    });
  }

  async function fetchViaRaw(gistId, filename) {
    const urls = gistRawUrls(gistId, filename);
    return firstOk(urls.map((url) => () => fetchJsonUrl(url)));
  }

  async function fetchViaApi(gistId, filename) {
    const res = await fetchWithTimeout(`https://api.github.com/gists/${gistId}`, 8000);
    if (!res.ok) throw new Error(`No se pudo leer el Gist (${res.status})`);
    ...
    if (!raw) throw new Error(`No se pudo descargar ${filename}`);
    return fetchJsonUrl(`${raw}${raw.includes("?") ? "&" : "?"}t=${Date.now()}`);
  }

  function fetchViaJsonp(gistId, filename) {
    return new Promise((resolve, reject) => {
      const cb = `gygGistCb${Date.now()}`;
      const script = document.createElement("script");
      let done = false;
      const timer = setTimeout(() => finish(new Error("Gist JSONP timeout")), 8000);
      ...
      window[cb] = (payload) => {
        try {
          const gist = payload && payload.data ? payload.data : payload;
          const file = gist && gist.files && gist.files[filename];
          if (!file) throw new Error(`No existe el archivo ${filename} en el Gist`);
          if (file.content && !file.truncated) {
            finish(null, JSON.parse(file.content));
            return;
          }
          throw new Error("Gist truncado");
        } catch (err) {
          finish(err);
        }
      };
      script.onerror = () => finish(new Error("Gist JSONP error"));
      script.src = `https://api.github.com/gists/${encodeURIComponent(gistId)}?callback=${cb}`;
  ...
  async function loadGistOnce(gistId, filename) {
    try {
      return await fetchViaRaw(gistId, filename);
    } catch (rawErr) {
      try {
        return await fetchViaApi(gistId, filename);
      } catch (apiErr) {
        try {
          return await fetchViaJsonp(gistId, filename);
        } catch (jsonpErr) {
          throw jsonpErr || apiErr || rawErr;
        }
      }
    }
  }

  async function fetchLiveStock(ids, filename) {
    let lastErr;
    for (let i = 0; i < ids.length; i += 1) {
      try {
        const raw = await loadGistOnce(ids[i], filename);
        writeStockCache(raw);
    ...
    throw lastErr || new Error("No se pudo leer el Gist");
  }

  async function loadStock() {
    const filename = cfg().GIST_FILENAME || "stock.json";
    const ids = gistIdCandidates();
    const ready = stockOnHand();
    if (!ids.length) {
      if (ready) return normalizeData(ready);
      const err = new Error("MISSING_GIST_ID");
      err.code = "MISSING_GIST_ID";
      throw err;
    }
    const live = fetchLiveStock(ids, filename);
    if (ready) {
      const winner = await Promise.race([live.catch(() => null), sleep(1800).then(() => null)]);
      if (winner) return winner;
      live.catch(() => {});
      return normalizeData(ready);
    }
    try {
      return await live;
    } catch (lastErr) {
      ...
      const err = new Error((lastErr && lastErr.message) || "No se pudo leer el Gist");
...
(function () {
  ...
  function esInstalador() {
    try {
      ...
      return hash === "#/conectar" || hash.indexOf("#/conectar?") === 0;
    } catch (_) {
      return false;
    }
  }

  function renderCatalogoCaido() {
    app.innerHTML = `
      <section class="empty-state" style="margin:3rem 1.25rem;max-width:36rem">
        <h1>El catálogo no se pudo cargar</h1>
        <p>Recargá la página en un rato. Si sigue, avisá al que te instaló el sistema.</p>
        <p>
    ...
    document.getElementById("gistRetryBtn")?.addEventListener("click", () => {
      ...
      try {
        sessionStorage.removeItem("gyg_gist_auto_retry");
      } catch (_) {}
      location.reload();
    });
  }
  ...
  async function init() {
    try {
      try {
        if ((window.GYG_CONFIG && String(window.GYG_CONFIG.GIST_ID || "").trim()) || document.getElementById("gyg-blogger-root")?.getAttribute("data-gist-id")) {
          localStorage.removeItem("gyg_gist_id");
        }
        sessionStorage.removeItem("gyg_gist_auto_retry");
      } catch (_) {}
      ...
      console.error(err);
      renderGistSetup(err);
      toast("No se pudo cargar el catálogo. Tocá Reintentar.", "error");
    }
  }
